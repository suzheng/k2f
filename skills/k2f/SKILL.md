...
6. **Look at the pixels.** `verify` only checks hashes. After pack, render a PNG and open the image. Watch empty top/bottom bands and type that is too large or too small; fix `page_config.margin` or role styles in `theme.json`, then relock. Commands: [writing.md](references/writing.md#visual-check).
