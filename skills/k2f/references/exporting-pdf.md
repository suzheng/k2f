...
`--scale` is `2`, `3`, or `4` (default **4**) — stamp pages only. Default export has the **same page count as the lock** (no integrity page, no source caption) — that is the customer PDF. `python3 scripts/check-pdf.py out.pdf` accepts that clean file (script lives under this skill’s `scripts/`). `--trust-pack` adds captions + a verify page; do not use it for customer delivery. Fillable `form_field` nodes become AcroForm widgets; `--flatten` paints lock glyphs instead.
