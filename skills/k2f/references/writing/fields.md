...
`content.type` is `text` | `math` | `image` | `container` | `table` | `form_field` (this skill does not author `code_block` or `table_reference` — see [package.md](package.md)). Layout `type` is `stack` | `grid` | `overlay` | `columns` (omit `type` → stack). Container `children` may be omitted (empty). Grid `rows` is optional (including one-row grids): omit → `{auto:true}` rows to fit children; `fr`/`pt` rows must be written. Details: [`nodes.schema.json`](../../schema/nodes.schema.json).
