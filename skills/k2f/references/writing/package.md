...
2. Under `root`, each top-level sibling is one slide. From the second sibling on, set `break_before: "page"` so a short face cannot share a sheet (same rule as card backs).
...
Same composed-page pattern as slides, on a small sheet:

1. `python scripts/init_package.py --workspace ./out/card --title "Card" --page card` → `252000×144000`, margin 0 (US 3.5×2"). Other sizes: `--width` / `--height` (keep `--page card` or `a4` for the unused preset).
2. Two root children (front, back). Each: copy [`ex_poster_shell.json`](../../catalog/content/ex_poster_shell.json) or [`ex_filled_page.json`](../../catalog/content/ex_filled_page.json); role `page_shell`; `break_inside: "avoid"`; `layout.height: 144000`. Back also `break_before: "page"`. `--expect-pages 2`.
3. Starter `page_shell` padding is 36pt — shrink that **role** if the inset is too large. Content box shorter than 180pt does not emit `LAYOUT_SLACK` / `PAGE_UNDERFILL` — do not pad with dummy copy.
...
2. **Footer not at the page bottom** (rare) — a vertical `stack`. Invoice/CV/letter-with-low-signature use the filled-page shell, not this.
...
| Title left + logo/meta/amount right | `ex_split_bar.json` — `{fr:1}` + `{auto:true}` (not overlay, not Flexbox). Right cell may be text (`variant: end` for amounts) |
...
- **Paged flow:** `break_inside: auto` (default) splits text **by wrapped line** and zero-padding vertical stacks **by child** when the page remainder is too small. `break_inside: avoid` never splits. `break_before: "page"` on a chapter, annex, signature, **slide 2+**, or **card back** — not a `p1`/`p2` page wrapper. `keep_with_next` keeps this node with the next sibling when both fit. Unsplittable siblings (overlay, `avoid`, padded card, grid, **padded columns**) move whole to the next page. Unpadded `columns` fill leftover height then continue. **Composed** vs **flow**: [writing.md](../writing.md#visual-check).
...
```bash
python scripts/init_package.py --workspace <workspace> --title "..." --page a4|letter|a4-landscape|widescreen|widescreen-43|square|portrait-45|card
# or --dir <source> for an existing author path
# slides: --page widescreen --margin 0
# posters: --page a4 --margin 0
# social: --page square | portrait-45
