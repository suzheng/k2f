...
| `TABLE_ROW_MISMATCH` | Sum of cell `colspan` (default 1) on each row equals `column_widths.length`; header_rows ≤ rows; table `column_widths` cannot be `{auto:true}` |
| `IMAGE_SIZE` | Provide image bytes + declared width/height millipt. Formats: PNG, JPEG, WebP, SVG — not GIF. |
| `SVG_TEXT` | SVG `<text>` / `<tspan>` / `<textPath>` / `<foreignObject>` fail at pack/unpack/compile (paint still fail-closed) — remove those tags; put labels in a K2F text node beside the image, or convert glyphs to `<path>`. Mentions inside XML comments or CDATA do not count. |
...
| `SCHEMA_INVALID` | Open [`schema/`](../../schema/) for the failing file. Key in schema but rejected → `pip install -U k2f`. Key **not** in schema → remove it (`space-between`, node-level `box_decoration`/`padding`/`text_align` are not keys). `colspan` is legal on **table cells** only (`COLSPAN_OUTSIDE_TABLE` otherwise). Image field is `src`, not `path`. Table `header_rows` is an integer count, not a cell array. Modifier objects need `range`, `type`, and `intent`. Optional layout keys (including grid `rows`; stack `type` defaults to stack) may be omitted or `null`. Theme roles: only `default` must set `font_family`, `font_size`, `line_height_mult`, `color`; other roles (including container/`rule`) inherit — omit unused keys, do not `null` them. |
...
| `INVALID_ARGUMENT` | Catch-all for many compile failures — read **message**. `root layout type … is not document flow` (grid, overlay, columns, or horizontal stack on root): keep starter `id: root` omitted/`stack` (vertical); paste the catalog fragment into `children`. Do not replace `content/root.json` with `ex_poster_shell.json` / `ex_grid.json` / `ex_overlay.json` / `ex_columns.json` / `ex_end_block.json`. |
...
- **Designed sheet (invoice/CV/flyer/poster/one-page infographic):** treat `PAGE_UNDERFILL` as must-fix even when `pack_verify.py` exits 0 — copy [`ex_filled_page.json`](../../catalog/content/ex_filled_page.json); leftover on table/notes `{fr:1}`.
- **Short letter / last page of a growing document:** empty bottom is allowed. `PAGE_UNDERFILL` is not emitted on the last page of a multi-page document.
