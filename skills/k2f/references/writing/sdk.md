...
| IDML from Editor | `export_idml_bytes` (package zip) / `export_idml_only_bytes` | `exportIdml` / `exportIdmlOnly` / `Editor.exportIdml`（当前内存包；要最新 lock 先 `save`） | `export_idml` / `export_idml_only` |
