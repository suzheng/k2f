...
| Filled page (invoice/CV/letter) | `content/ex_filled_page.json` | Same shell as poster; `{fr:1}` grower holds a **table**, totals in the `{auto:true}` footer (label\|amount → `ex_split_bar`). Not poster-only. **Change `height`**. | starter (`page_shell`, `h1`, `table`, `table_header_cell`, `table_row_cell`, `body`) |
