...
`ex_poster_shell.json`, `ex_poster_growers.json`, and `ex_cover.json` ship `layout.height: 240000` as a **demo**. For a real page, set `height` to the **content box** (page height − margins). A4 / margin 0 → `842000`. Widescreen / margin 0 → `960000×540000` with height `540000`. Use the table in [package.md](../references/writing/package.md) (`595000×842000`), not ISO `595280×841890`. Full-bleed shells copy `ex_poster_shell.json` as role `page_shell` (starter inset `padding_pt: 36000`); do not pad `section`/`body`.
