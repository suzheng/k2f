...
| Header bar (title left, logo/amount right) | `content/ex_split_bar.json` | 2-col grid `{fr:1}` + `{auto:true}` — right cell is an image **or** amount text (`variant: end`); not Flexbox `space-between`, not overlay | starter (`h2`, `image`) |
...
| Fixed-height grid + `fr` | `content/ex_grid.json` | `columns`/`rows` tracks, `height`, `cell_align`, `row_gap`, `column_gap`. N-col matrix: N `{fr:1}` columns; omit `rows` to wrap | starter |
...
| Filled page (invoice/CV/letter / one-page infographic) | `content/ex_filled_page.json` | Same shell as poster; `{fr:1}` grower holds a **table**, totals in the `{auto:true}` footer (label\|amount → `ex_split_bar`). Not poster-only. **Change `height`**. | starter (`page_shell`, `h1`, `table`, `table_header_cell`, `table_row_cell`, `body`) |
...
| Equal-height card interiors | `content/ex_card_bands.json` | Stretched row + each card inner `{auto,fr,auto}` (pin ends). Not stack `start` / `space-between`. Sparse KPI → stack `justify_content: center` | starter (`card`, `h2`, `body`) |
...
| Fillable form | `content/ex_form.json` | `role: form_field` + `content.type: form_field` — reserved box (label+underline, multiline, checkbox, signature name/date). Empty value still occupies the slot; do **not** draw `____` | starter (`form_field`, `signature_block`) |
...
By deliverable (same files, not extra packages): invoice / CV / letter → `ex_filled_page` (role `page_shell`; fix `height`); header/totals → `ex_split_bar` + cell `variant: end`; dark sheet → `paper` / document fill + `ex_on_dark` (zero `margin` only for full-bleed); one-page infographic/checklist (even if titled report) → `ex_filled_page` / `ex_poster_shell` (fix `height`); long report → heading + stack + list + table ± `ex_on_dark`; flowing two-column **article** → `ex_columns` (one container; figures `column_span: all`); magazine figure+copy → `ex_media_row`; one-page poster, slide, or social square → `ex_poster_shell` (role `page_shell`; `--page square` height `600000`) ± `ex_poster_growers` ± `ex_card_bands` ± `ex_overlay` ± `ex_glass`; N-col matrix → omit `rows`; thesis cover → `ex_cover` (fix `height`); duplex card → `--page card`, two shells, back `break_before: page`, `--expect-pages 2`; title+logo / amount bar → `ex_split_bar`; letter sender / right-flush cell → `ex_end_block`; divider → `ex_rule` (vertical stretch parent); fillable application / contract blanks → `ex_form` (`form_field`, never `____`); form-like table rules → `ex_table_edges`; dense metrics → `ex_table_dense`; badge/pill → `ex_badge` (copy the stack wrapper); running header/footer split → `catalog/manifest.json`.
