...
- `export-docx` / `export-pptx`: disable text-frame wrap when the lock is a single line that already fills the box. Host bold/metrics that are wider than rustybuzz were wrapping the last word onto a second line that the one-line box then clipped. Multi-line lock boxes and frames with leftover width still wrap.
- `export-docx` / `export-pptx`: disable text-frame wrap when the lock is a single line that already fills the box. Host bold/metrics that are wider than rustybuzz were wrapping the last word onto a second line that the one-line box then clipped. Multi-line lock boxes and frames with leftover width still wrap.
