use super::detect::{box_belongs_to_effect, is_full_page};
...
/// Drop body text, content images, and native-simple boxes from a page's ops.
...
fn is_leading_page_bg(op: &PaintOp, keep: &ChromeKeep) -> bool {
    ...
    match (keep.page_width, keep.page_height) {
        (Some(w), Some(h)) => is_full_page(w, h, rect),
        _ => true,
    }
}
