...
pub fn expand_rect_for_shadow(rect: Rect, decoration: &BoxDecoration) -> Result<Rect, PptxError> {
    ...
    let layers = match shadow {
        ...
        ShadowRef::Ref(name) => {
            return Err(PptxError::Write(format!("unresolved shadow ref '{name}'")));
        }
    };
    if layers.is_empty() {
        return Ok(rect);
...
pub fn crop_rgb_to_png(
    ...
    let h = img_h as i32;
    if w <= 0 || h <= 0 {
        return Err(PptxError::Write("empty page bitmap".into()));
    }
...
fn encode_png(w: u32, h: u32, rgb: &[u8]) -> Result<Vec<u8>, PptxError> {
