...
- **Registry packages** — `pip install k2f` (PyPI), `npm i @openk2f/k2f` (npm), `cargo install k2f` (crates.io); **0.3.x** on all three (npm uses scoped name `@openk2f/k2f` because unscoped `k2f` is blocked).
...
Format spec [v0.3](../spec/k2f-v0.3.md) is the on-disk contract, aligned with SDK **0.3.x**. Patch SDK releases keep this spec unless schemas or the lock format break. See [COMPATIBILITY.md](../../COMPATIBILITY.md) for `engine_version` matching on locks.
