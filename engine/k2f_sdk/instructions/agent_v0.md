...
- If the source is Markdown, call markdown_to_k2f. Do not hand-build a tree from Markdown.
- Pick a named template: blank, invoice, legal, clinical_summary, or report. Use copy_template or Editor.open_template. Do not write theme JSON, paint, lock, x/y, CSS, or font sizes.
- Page size is A4 or Letter (set in the template manifest).
...
API: copy_template(template, dest), resolve_template(name), official_templates(), Editor.open_template(template), Editor.open_dir(path), Editor.open_bytes(bytes), insert_node(parent_id, index, node_json), replace_text(id, text), set_role(id, role, variant?), delete_node(id), set_running_header(text), set_running_footer(text), get_node(id), save_bytes(), save_dir(path), markdown_to_k2f(md, title, template), k2f_to_markdown(bytes). Sign separately with sign(bytes, key).
